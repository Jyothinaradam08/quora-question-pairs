...
# Ensure NLTK stopwords dataset is downloaded silently
nltk.download('stopwords', quiet=True)
STOP_WORDS = set(stopwords.words('english'))

# Dynamically construct directory path to load pickle files safely
...
def test_fetch_length_features(q1, q2):
    ...
    # Longest Common Substring ratio with fallback protection
    try:
...
def preprocess(q):
    ...
    # Replace numbers with string scale abbreviations
...
def query_point_creator(q1, q2):
    ...
    input_query.append(round(common_words / total_words, 2) if total_words > 0 else 0.0)
